...
// 테이블의 전체 레코드수만 얻음
$sql = "select count(1) as cnt from (select * from (select od_id {$sql_common} group by a.od_id) as b ) c LEFT JOIN bomiora_shop_cart d ON c.od_id = d.od_id WHERE d.ct_status <> '취소' ";
//var_dump($sql);
...
LEFT JOIN {$g5['g5_shop_cart_table']} d ON c.od_id = d.od_id WHERE d.ct_status <> '취소' ";
//var_dump($sql);
//exit;
