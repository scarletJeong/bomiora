...
$mb_id							= isset($_POST['inf_mb_id']) ? clean_xss_tags(trim($_POST['inf_mb_id']), 1, 1) : clean_xss_tags(trim($_POST['mb_id']), 1, 1);// 인플루언서아이디(인플루언서 아이디값이 넘어왔다면 mb_id 값을 무시고하고 삽입한다.

// 인플루언서 아이디가 존재한다면 종류를 무조건 서포트로 한다.
if($_POST['inf_mb_id'] && $is_rvkind == 'general') {
	alert('인플루언서 아이디가 존재한다면 리뷰종류는 서포트여야 합니다.');
}
...
if ($w == "") {
	$sql = " insert into {$g5['g5_shop_item_use_table']} set it_id = '{$it_id}', is_time = '" . G5_TIME_YMDHIS . "', is_ip = '{$_SERVER['REMOTE_ADDR']}', {$sql_common} ";
	sql_query($sql);
