...
<script>
  $(function() {
    ...
    $("#sch_member").click(function(e) {
      ...
      var _this = $(this),
      mb_ids = _this.siblings("input[name=mb_id]:first").val().replace(/ /g,'');
  ...
  function form_check(f) {
    ...
    if (f.chk_all_mb.checked && (f.cp_alimtalk_send.checked || f.cp_email_send.checked)) {
      alert("전체회원인 경우에는 알림이 발송되지 않습니다.");
      f.cp_alimtalk_send.checked = false;
      f.cp_email_send.checked = false;
      return false;
    }
