...
$sel_field = (isset($_GET['sel_field']) && in_array($_GET['sel_field'], array('od_id', 'mb_id', 'od_name', 'od_tel', 'od_hp', 'od_b_name', 'od_b_tel', 'od_b_hp', 'od_deposit_name', 'od_invoice', 'it_name', 'it_id')) ) ? $_GET['sel_field'] : '';
...
if ($search != "") {
    if ($sel_field != "") {
      if ($sel_field == 'it_name' || $sel_field == 'it_id') {
        $join_as = 't1.';
        $join_on = " AS t1 LEFT JOIN bomiora_shop_cart AS t2 ON t1.od_id = t2.od_id ";
        if ($sel_field == 'it_id') {
          $where[] = " t2.${sel_field} = '$search' ";
        } else {
          $where[] = " t2.${sel_field} like '%$search%' ";
        }
      } else {
...
if ($od_status) {
    switch($od_status) {
        case '전체취소':
            $where[] = " ${join_as}od_status = '취소' ";
            break;
        case '부분취소':
            $where[] = " ${join_as}od_status IN('주문', '입금', '준비', '배송', '완료') and ${join_as}od_cancel_price > 0 ";
            break;
        case '취소제외정산':
            $where[] = " ${join_as}od_status IN('배송', '완료') ";
            break;            
        default:
            $where[] = " ${join_as}od_status = '$od_status' ";
            break;
    }
...
if ($od_settle_case) {
    if( $od_settle_case === '간편결제' ) {
        $where[] = " ${join_as}od_settle_case in ('간편결제', '삼성페이', 'lpay', 'inicis_kakaopay') ";
    } else {
        $where[] = " ${join_as}od_settle_case = '$od_settle_case' ";
    }
}

if ($od_misu) {
    $where[] = " ${join_as}od_misu != 0 ";
}

if ($od_cancel_price) {
    $where[] = " ${join_as}od_cancel_price != 0 ";
}

if ($od_refund_price) {
    $where[] = " ${join_as}od_refund_price != 0 ";
}

if ($od_receipt_point) {
    $where[] = " ${join_as}od_receipt_point != 0 ";
}

if ($od_coupon) {
    $where[] = " ( ${join_as}od_cart_coupon > 0 or ${join_as}od_coupon > 0 or ${join_as}od_send_coupon > 0 ) ";
}

if ($od_escrow) {
    $where[] = " ${join_as}od_escrow = 1 ";
}

if ($fr_date && $to_date) {
    $where[] = " ${join_as}od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
}

if ($where) {
    $sql_search = $join_on . ' where '.implode(' and ', $where);
}

//if ($sel_field == "")  $sel_field = "od_id";
if ($sel_field == "")  $sel_field = "it_id";
...
if ($join_as) {
  $sql = " select count(c.od_id) AS cnt from (select ${join_as}od_id ${sql_common} GROUP BY ${join_as}od_id) AS c";
} else {
...
$sql  = " select ${join_as}*,
            (${join_as}od_cart_coupon + ${join_as}od_coupon + ${join_as}od_send_coupon) as couponprice
           $sql_common
           group by ${join_as}od_id
           order by ${join_as}$sort1 $sort2
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" rowspan="3">처방여부</th>
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td headers="th_ordnum" class="td_odrnum2" rowspan="2" colspan="2">
			...
			<?php
			// 상품목록
			$sql_ct = " select it_id, it_name, cp_price, ct_notax, ct_send_cost, it_sc_type from {$g5['g5_shop_cart_table']} where od_id = '{$row['od_id']}' group by it_id order by ct_id ";
			$result_ct = sql_query($sql_ct);
			for($c=0; $row_ct=sql_fetch_array($result_ct); $c++) {

	            // 상품의 옵션정보
		        $sql_op = " select ct_id, it_id, ct_price, ct_qty, ct_option, ct_status, cp_price, ct_send_cost, io_type, io_price
			                from {$g5['g5_shop_cart_table']}
				            where od_id = '{$row['od_id']}'
					          and it_id = '{$row_ct['it_id']}'
						    order by io_type asc, ct_id asc ";
	            $res = sql_query($sql_op);
		        $rowspan = sql_num_rows($res);

				echo "<br>".stripslashes($row_ct['it_name'])." | ";
        ...
        <td rowspan="3" class="td_name"><?php echo $is_prescribed; ?></td>
    ...
    <tr class="orderlist">
        ...
        <td><?php echo ('처방 ' . number_format($total_prescibed)); ?>건</td>
        <td></td>
    </tr>
    </tfoot>
    </table>
...
<?php if (($od_status == '' || $od_status == '완료' || $od_status == '전체취소' || $od_status == '부분취소' || $od_status == '취소제외정산') == false) {
...
function search_result(e, sel_field, search) {
  ...
  form.append( $('<input>', { type: 'hidden', name: 'sel_field', val: sel_field }) ); 
  form.append( $('<input>', { type: 'hidden', name: 'search', val: search }) ); 
