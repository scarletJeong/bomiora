...
$bn_position = (isset($_GET['bn_position']) && in_array($_GET['bn_position'], $_const['banner_position'])) ? $_GET['bn_position'] : '';
...
<div class="local_ov01 local_ov">
    ...
    <select name="bn_position" id="bn_position">
		...
		<option value="<?php echo $key ?>"<?php echo get_selected($key, $bn_position, true) ?>><?php echo $value ?></option>
		<?php } ?>
    </select>
<!--
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        //pc 이미지
		$pcimg = G5_DATA_PATH.'/banner/pc_'.$row['bn_id'];
		if(file_exists($pcimg)) {
			$size = @getimagesize($pcimg);
			if($size[0] && $size[0] > 250)
				$width = 250;
			else
				$width = $size[0];
			$pc_img = "";
			$pc_img .= '<img src="'.G5_DATA_URL.'/banner/pc_'.$row['bn_id'].'?'.preg_replace('/[^0-9]/i', '', $row['bn_time']).'" width="'.$width.'" alt="'.strip_tags(clean_xss_attributes($row['bn_alt'])).'">';
		}
		//mobile 이미지
		$moimg = G5_DATA_PATH.'/banner/mo_'.$row['bn_id'];
		if(file_exists($moimg)) {
			$size = @getimagesize($moimg);
			if($size[0] && $size[0] > 250)
				$width = 250;
			else
				$width = $size[0];
			$mo_img = "";
			$mo_img .= '<img src="'.G5_DATA_URL.'/banner/mo_'.$row['bn_id'].'?'.preg_replace('/[^0-9]/i', '', $row['bn_time']).'" width="'.$width.'" alt="'.strip_tags(clean_xss_attributes($row['bn_alt'])).'">';
		}
		/*
    ...
    <tr class="<?php echo $bg; ?>">
        <td class="td_num"><?php echo $row['bn_id'] ?></td>
		<td class="td_img_view sbn_img"><?php echo $pc_img ?></td>
		<td class="td_img_view sbn_img"><?php echo $mo_img ?></td>
        <td headers="th_loc"><?php echo $_const['banner_position'][$row['bn_position']] ?></td>
		<td headers="th_loc"><?php echo $row['bn_skin'] ?></td>
