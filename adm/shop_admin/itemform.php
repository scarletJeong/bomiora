...
if ($w == "")
{
	...
	$it['it_weight'] = stripslashes(get_cookie("ck_weight"));
	$it['it_prescription'] = stripslashes(get_cookie("ck_prescription"));
	$it['it_takeway'] = stripslashes(get_cookie("ck_takeway"));
	$it['it_package'] = stripslashes(get_cookie("ck_package"));
	$it['it_kind'] = 'general';// 상품종류(일반상품)
	$it['it_shipping_process'] = $default['de_shipping_process_content'];// 처방 프로세스
	$it['it_faq'] = $default['de_faq_content'];// FAQ
	$it['it_change_content'] = $default['de_change_content'];// 교환/환불
	$it['it_baesong_content'] = $default['de_baesong_content'];// 배송
}
else if ($w == "u")
{
    $html_title .= "수정";
...
<li><a href="#anc_sitfrm_banner">배너설정</a></li>
</ul>
';
...
// 원본 상품코드 선택 추가 jacknam
$it_org_id_name = '';
$it_org_id_select = '';

if ($it['ca_id'] == '40') {
  if ($it['it_org_id']) {
    $sql = "SELECT it_id, it_name FROM {$g5['g5_shop_item_table']} WHERE it_id = '{$it['it_org_id']}' limit 1";
    $row = sql_fetch($sql);
    if (isset($row['it_id']) && isset($row['it_name'])) {
      $it_org_id_name = ['it_id' => $row['it_id'], 'it_name' => $row['it_name']];
    }
  }
} else {
  $sql = "SELECT it_id, it_name FROM {$g5['g5_shop_item_table']} a, {$g5['g5_shop_category_table']} b WHERE a.ca_id = b.ca_id AND b.ca_id <> '40' ";
  if ($it['ca_id'] != '' && $it['it_id'] != '') {
    $sql .= "AND a.ca_id = '" . $it['ca_id'] . "' AND a.it_id <> '" . $it['it_id'] . "'";
  }
  $sql .= " ORDER BY it_id ";
  $result = sql_query($sql);
  for ($i=0; $row=sql_fetch_array($result); $i++) {
    $it_org_id_select .= "<option value=\"{$row['it_id']}\">{$row['it_id']}&nbsp;&nbsp;{$row['it_name']}</option>";
  }
}
if (!$it_org_id_name && !$it_org_id_select) {
  $it_org_id_name = ['it_id' => '', 'it_name' => ''];
}
?>
            ...
            <td>
              ...
              <?php if ($it['ca_id'] == '40') { ?>
                <select name="ca_id" id="ca_id">
                    <option value="<?php echo $it['ca_id']; ?>" selected>파생상품</option>
                </select>
              <?php } else { ?>
...
<section id="anc_sitfrm_ini">
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		<tr>
            ...
            <td colspan="2">
                <?php// echo help(""); ?>
				<input type="radio" name="it_kind" value="general" id="it_kind_general" <?php if ($it['it_kind'] == 'general') { echo 'checked="checked"'; } ?>>
				<label for="it_kind_general">일반상품</label>
				<input type="radio" name="it_kind" value="prescription" id="mb_certify_prescription" <?php if ($it['it_kind'] == 'prescription') { echo 'checked="checked"'; } ?>>
				<label for="mb_certify_prescription">처방상품</label>
            </td>
        </tr>
		<!--//-->
		<!-- 추가 : jacknam -->
		<tr>
            <th scope="row">원본 상품코드</th>
            <td colspan="2">
              <?php if ($it_org_id_name) { ?>
              <input type="hidden" name="it_org_id" id="it_org_id" value="<?php echo $it_org_id_name['it_id']; ?>">
              <span class="frm_ca_id"><?php echo $it_org_id_name['it_id'] ? "{$it_org_id_name['it_id']} ({$it_org_id_name['it_name']})" : "해당없음"; ?></span>
              <?php } else { ?>
                <?php echo help("원본 상품코드를 선택하면 리뷰 및 분류가 원본 상품과 통합됩니다."); ?>
                <select name="it_org_id" id="it_org_id">
                    <option value="">없음</option>
                    <?php echo conv_selected_option($it_org_id_select, $it['it_org_id']); ?>
                </select>
              <?php } ?>
            </td>
        </tr>
		...
		<tr>
            <th scope="row"><label for="it_subject">제목</label></th>
            <td>
                <?php echo help("제품목록에서 제품명 상단에 들어가는 내용입니다."); ?>
                <input type="text" name="it_subject" value="<?php echo get_text(cut_str($it['it_subject'], 250, "")); ?>" id="it_subject" class="frm_input" size="95">
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_subject" value="1" id="chk_ca_it_subject">
                <label for="chk_ca_it_subject">분류적용</label>
                <input type="checkbox" name="chk_all_it_subject" value="1" id="chk_all_it_subject">
                <label for="chk_all_it_subject">전체적용</label>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <label for="it_type1">베스트 <img src="<?php echo G5_SHOP_URL; ?>/img/icon_hit.gif" alt=""></label>
                ...
                <label for="it_type4">인기(메인) <img src="<?php echo G5_SHOP_URL; ?>/img/icon_best.gif" alt=""></label>
		...
		<tr>
            ...
            <td>
                <?php// echo help(""); ?>
                <input type="checkbox" name="it_icon1" value="1" <?php echo ($it['it_icon1'] ? "checked" : ""); ?> id="it_icon1">
                <label for="it_icon1">한의약품</label>
                <input type="checkbox" name="it_icon2" value="1" <?php echo ($it['it_icon2'] ? "checked" : ""); ?> id="it_icon2">
                <label for="it_icon2">오늘의 배송</label>
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_icon" value="1" id="chk_ca_it_icon">
                <label for="chk_ca_it_icon">분류적용</label>
                <input type="checkbox" name="chk_all_it_icon" value="1" id="chk_all_it_icon">
                <label for="chk_all_it_icon">전체적용</label>
            </td>
        </tr>
		<tr>
            <th scope="row"><label for="it_weight">중량/용량</label></th>
            <td>
                <?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
                <input type="text" name="it_weight" value="<?php echo get_text($it['it_weight']); ?>" id="it_weight" class="frm_input" size="40">
            </td>
            <td class="td_grpset">
		...
		<tr>
            <th scope="row"><label for="it_prescription">처방단위</label></th>
            <td>
                <?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
                <input type="text" name="it_prescription" value="<?php echo get_text($it['it_prescription']); ?>" id="it_prescription" class="frm_input" size="40">
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_prescription" value="1" id="chk_ca_it_prescription">
                <label for="chk_ca_it_prescription">분류적용</label>
                <input type="checkbox" name="chk_all_it_prescription" value="1" id="chk_all_it_prescription">
                <label for="chk_all_it_prescription">전체적용</label>
            </td>
        </tr>
		<tr>
            <th scope="row"><label for="it_takeway">복용방법</label></th>
            <td>
                <?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
                <input type="text" name="it_takeway" value="<?php echo get_text($it['it_takeway']); ?>" id="it_takeway" class="frm_input" size="40">
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_takeway" value="1" id="chk_ca_it_takeway">
                <label for="chk_ca_it_takeway">분류적용</label>
                <input type="checkbox" name="chk_all_it_takeway" value="1" id="chk_all_it_takeway">
                <label for="chk_all_it_takeway">전체적용</label>
            </td>
        </tr>
		<tr>
            <th scope="row"><label for="it_package">패키지구성</label></th>
            <td>
                <?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
                <input type="text" name="it_package" value="<?php echo get_text($it['it_package']); ?>" id="it_package" class="frm_input" size="40">
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_package" value="1" id="chk_ca_it_package">
                <label for="chk_ca_it_package">분류적용</label>
                <input type="checkbox" name="chk_all_it_package" value="1" id="chk_all_it_package">
                <label for="chk_all_it_package">전체적용</label>
            </td>
        </tr>
        <tr>
		...
		<tr>
            ...
            <td colspan="2">
			<?php echo help("배송비 설정은 별도로 하셔야 합니다. 여기는 사용자에게 보여지는 내용 입력창입니다."); ?>
			<?php echo editor_html('it_baesong_content', get_text(html_purifier($it['it_baesong_content']), 0)); ?></td>
        </tr>
		<tr>
            <th scope="row"><label for="it_precautions">주의사항</label></th>
            <td colspan="2">
				<?php echo help('주의사항과 주의사항 사이는 | 로 구분하세요. (예: 과다복용주의|노약자금지) 첫자로 #은 입력하지 마세요. (예: #과다복용주의|#노약자금지 [X])'."\n".'분류명에 일부 특수문자 ()/ 는 사용할수 없습니다.'); ?>
                <input type="text" name="it_precautions" value="<?php echo get_text(html_purifier($it['it_precautions'])); ?>" id="it_precautions" maxlength="255" class="frm_input frm_input_full">
            </td>
        </tr>
		<tr>
            <th scope="row">기관인증</th>
            <td>
                <?php //echo help(""); ?>
				<?php
				$idnum = 1;
				foreach ($_const['certification'] as $key => $val) {
				?>
				<input type="checkbox" name="it_certification<?php echo $idnum ?>" value="1" <?php echo ($it['it_certification'.$idnum] ? "checked" : ""); ?> id="it_certification<?php echo $idnum ?>">
                <label for="it_certification<?php echo $idnum ?>"><?php echo $val ?><!--<img src="<?php echo G5_IMG_URL?>/detail/<?php echo $key ?>" alt="<?php echo $val ?>">--></label>
				<?php
					$idnum++;
				}
				?>
            ...
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_certification" value="1" id="chk_ca_it_certification">
                <label for="chk_ca_it_certification">분류적용</label>
                <input type="checkbox" name="chk_all_it_certification" value="1" id="chk_all_it_certification">
                <label for="chk_all_it_certification">전체적용</label>
            </td>
        </tr>
		<tr>
            <th scope="row">처방 프로세스</th>
            <td colspan="2"> <?php echo editor_html('it_shipping_process', get_text(html_purifier($it['it_shipping_process']), 0)); ?></td>
        </tr>
		<tr>
            <th scope="row">FAQ</th>
            <td colspan="2"> <?php echo editor_html('it_faq', get_text(html_purifier($it['it_faq']), 0)); ?></td>
        </tr>
		<tr>
            <th scope="row">교환/환불</th>
            <td colspan="2"> <?php echo editor_html('it_change_content', get_text(html_purifier($it['it_change_content']), 0)); ?></td>
        </tr>
        <tr>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
			<tr>
				...
				<td>
					<input type="file" name="it_bn_pcimg" id="it_bn_pcimg" accept="image/gif, image/jpeg, image/png">
					<?php
					$pcimg_str = "";
					$bimg = G5_DATA_PATH."/banner_item/pc_{$it['it_id']}";

					if (file_exists($bimg) && $it['it_id']) {

						$size = @getimagesize($bimg);
						if($size[0] && $size[0] > 450)
							$width = 450;
						else
							$width = $size[0];

						echo '<input type="checkbox" name="it_bn_pcimg_del" value="1" id="it_bn_pcimg_del"> <label for="it_bn_pcimg_del">삭제</label>';
						$pcimg_str = '<img src="'.G5_DATA_URL.'/banner_item/pc_'.$it['it_id'].'" width="'.$width.'">';
					}
					if ($pcimg_str) {
			...
			<tr>
				...
				<td>
					<input type="file" name="it_bn_moimg" id="it_bn_moimg" accept="image/gif, image/jpeg, image/png">
					<?php
					$moimg_str = "";
					$bimg = G5_DATA_PATH."/banner_item/mo_{$it['it_id']}";
					if (file_exists($bimg) && $it['it_id']) {
						$size = @getimagesize($bimg);
						if($size[0] && $size[0] > 450)
							$width = 450;
							...
							$width = $size[0];
						echo '<input type="checkbox" name="it_bn_moimg_del" value="1" id="it_bn_moimg_del"> <label for="it_bn_moimg_del">삭제</label>';
						$moimg_str = '<img src="'.G5_DATA_URL.'/banner_item/mo_'.$it['it_id'].'" width="'.$width.'">';
					}
					if ($moimg_str) {
						echo '<div class="">';//banner_or_img
						echo $moimg_str;
			...
			<tr>
				<th scope="row"><label for="it_bn_alt">이미지 설명</label></th>
				<td>
					<input type="text" name="it_bn_alt" value="<?php echo get_text($it['it_bn_alt']); ?>" id="it_bn_alt" class="frm_input" size="80">
				</td>
			</tr>
			<tr>
				<th scope="row"><label for="it_bn_url">링크</label></th>
				<td>
					<?php echo help("배너클릭시 이동하는 주소입니다."); ?>
					<input type="text" name="it_bn_url" size="80" value="<?php echo get_sanitize_input($it['it_bn_url']); ?>" id="it_bn_url" class="frm_input">
				</td>
			</tr>
			<tr>
				<th scope="row"><label for="it_bn_new_win">새창</label></th>
				<td>
					<?php echo help("배너클릭시 새창을 띄울지를 설정합니다.", 50); ?>
					<select name="it_bn_new_win" id="it_bn_new_win">
						<option value="0" <?php echo get_selected($it['it_bn_new_win'], 0); ?>>사용안함</option>
						<option value="1" <?php echo get_selected($it['it_bn_new_win'], 1); ?>>사용</option>
					</select>
				</td>
			</tr>
        </tbody>
...
function fitemformcheck(f)
{
	...
	<?php echo get_editor_js('it_shipping_process'); ?>
	<?php echo get_editor_js('it_faq'); ?>
	<?php echo get_editor_js('it_change_content'); ?>
	<?php echo get_editor_js('it_baesong_content'); ?>
