...
// 포인트 적립 및 주문 통합 설정필드 추가
if(!isset($default['de_bonus_point'])) {
  sql_query(" ALTER TABLE `{$g5['g5_shop_default_table']}`
  ADD `de_bonus_point` tinyint(4) NOT NULL DEFAULT '0' AFTER `de_settle_point_unit`,
  ADD `de_point_timing` tinyint(4) NOT NULL DEFAULT '0' AFTER `de_bonus_point`,
  ADD `de_point_rate` decimal(3,1) NOT NULL DEFAULT '1.0' AFTER `de_point_timing`,
  ADD `de_order_pack` tinyint(4) NOT NULL DEFAULT '0' AFTER `de_point_rate`,
  ADD `de_pack_timing` tinyint(4) NOT NULL DEFAULT '0' AFTER `de_order_pack` ", true);
}
?>
...
<section id ="anc_scf_payment">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row"><label for="de_bonus_point">포인트 적립</label></th>
            <td>
                <?php echo help("신용카드, 계좌이체, 가상계좌 결제시 고객에게 포인트 적립을 제공할지를 설정합니다. (기본값은 '아니오')"); ?>
                <select id="de_bonus_point" name="de_bonus_point">
                    <option value="0" <?php echo get_selected($default['de_bonus_point'], 0); ?>>아니오</option>
                    <option value="1" <?php echo get_selected($default['de_bonus_point'], 1); ?>>예</option>
                </select>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="de_point_timing">포인트 적립 시기</label></th>
            <td>
                <?php echo help("포인트 적립을 제공할 경우 제공하는 시점을 설정합니다. (기본값은 '상품 배송 시')"); ?>
                <select id="de_point_timing" name="de_point_timing">
                    <option value="0" <?php echo get_selected($default['de_point_timing'], 0); ?>>상품 배송 시</option>
                    <option value="1" <?php echo get_selected($default['de_point_timing'], 1); ?>>고객 결제 시</option>
                </select>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="de_point_rate">포인트 적립 비율</label></th>
            <td>
                <?php echo help("결제 금액 대비 제공할 적립 포인트의 비율을 설정합니다. %를 제외하고 소수점을 포함한 실수를 입력하세요. (기본값은 '1.0')"); ?>
                결제 금액 대비 <input type="text" name="de_point_rate" value="<?php echo ($default['de_point_rate'] ? get_sanitize_input($default['de_point_rate']) : 1); ?>" id="de_point_rate" class="frm_input" size="2"> % (배송비 제외)
            </td>
        </tr>
        <!-- //
        <tr>
            <th scope="row"><label for="de_order_pack">주문 통합</label></th>
            <td>
                <?php echo help("고객 결제시 동일 고객의 미완료 된 주문이 있을 경우 이를 통합하여 제공합니다. (기본값은 '아니오')\n통합할 경우 하나의 주문번호로 합쳐져 진료 및 배송이 통합 진행됩니다."); ?>
                <select id="de_order_pack" name="de_order_pack">
                    <option value="0" <?php echo get_selected($default['de_order_pack'], 0); ?>>아니오</option>
                    <option value="1" <?php echo get_selected($default['de_order_pack'], 1); ?>>예</option>
                </select>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="de_pack_timing">통합 시 예약 시간 기준</label></th>
            <td>
                <?php echo help("주문 통합 시 진료 예약 시간 통합 기준을 설정합니다. (기본값은 '빠른 시간')"); ?>
                <select id="de_pack_timing" name="de_pack_timing">
                    <option value="0" <?php echo get_selected($default['de_pack_timing'], 0); ?>>빠른 시간</option>
                    <option value="1" <?php echo get_selected($default['de_pack_timing'], 1); ?>>느린 시간</option>
                </select>
            </td>
        </tr>
        -->
...
<section id="anc_scf_delivery">
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		<tr>
            ...
            <td><?php echo editor_html('de_shipping_process_content', get_text(html_purifier($default['de_shipping_process_content']), 0)); ?></td>
        </tr>
		<tr>
            <th scope="row">FAQ</th>
            <td><?php echo editor_html('de_faq_content', get_text(html_purifier($default['de_faq_content']), 0)); ?></td>
        </tr>
        <tr>
...
<section id="anc_scf_sms" >
    ...
    <section id="scf_sms_pre">
        ...
        <div id="scf_sms">
            <?php
            $scf_sms_title = array (1=>"회원가입시 고객님께 발송", "주문시 고객님께 발송(일반)", "주문시 관리자에게 발송", "입금확인시 고객님께 발송", "상품배송시 고객님께 발송", "주문시 고객님께 발송(처방)");
            for ($i=1; $i<=6; $i++) {
            ?>
...
function fconfig_check(f)
{
	...
	<?php echo get_editor_js('de_shipping_process_content'); ?>
	<?php echo get_editor_js('de_faq_content'); ?>
...
$(function() {
    ...
    $(document).ready(function () {
        ...
        function hash_goto_scroll(hash){
            var $elem = hash ? $("#"+hash) : (window.location.hash ? $('#' + window.location.hash.replace('#', '')) : []);
            if($elem.length) {
