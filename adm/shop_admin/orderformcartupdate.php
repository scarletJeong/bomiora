...
if(is_array($arr_it_id) && !empty($arr_it_id)) {
    ...
    foreach($unq_it_id as $it_id) {
		...
		// 프로필 부분 처리
		$sql4 = " update {$g5['g5_shop_health_profile_cart_table']} set ct_status = '$ct_status' where od_id = '$od_id' and it_id = '$it_id' ";
		sql_query($sql4);
    }
}
...
if (in_array($_POST['ct_status'], $status_cancel)) {
    ...
    if($row['od_count1'] == $row['od_count2']) {
        ...
        // PG 신용카드 결제 취소일 때
        //if($pg_cancel == 1 && order_cancelable($row['od_id'])) {
        if($pg_cancel == 1) {
            ...
            if($od['od_tno'] && (
              $od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '간편결제' || $od['od_settle_case'] == 'KAKAOPAY'
             ...
             || ($od['od_pg'] == 'inicis' && is_inicis_order_pay($od['od_settle_case']))
                ...
                // PG 취소요청 성공했으면
                if($pg_res_cd == '') {
                    $pg_cancel_log = ' PG 결제 승인취소 처리';
                    $sql = " update {$g5['g5_shop_order_table']}
                                set od_refund_price = '{$od['od_receipt_price']}', od_receipt_price = '0'
                                where od_id = '$od_id' ";
                    sql_query($sql);
                    
                    post_order_cancel($od_id);
                }
            }
        }
