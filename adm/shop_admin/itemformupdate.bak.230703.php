...
@mkdir(G5_DATA_PATH."/banner_item", G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH."/banner_item", G5_DIR_PERMISSION);
...
// 주의사항에 & 나 = 는 사용이 불가하므로 2바이트로 바꾼다.
/*
$src_char = array('&', '=');
$dst_char = array('＆', '〓');
$it_precautions_list = isset($_POST['it_precautions_list']) ? str_replace($src_char, $dst_char, $_POST['it_precautions_list']) : '';
//https://github.com/gnuboard/gnuboard5/commit/f5f4925d4eb28ba1af728e1065fc2bdd9ce1da58 에 따른 조치
$str_it_precautions_list = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", (string)$it_precautions_list);
$it_precautions_list = preg_replace('#<script(.*?)>(.*?)<\/script>#is', '', $it_precautions_list);
*/

$it_precautions = preg_replace('#<script(.*?)>(.*?)<\/script>#is', '', $it_precautions);
...
/* 배너 관련 */
$it_bn_pcimg		= isset($_FILES['it_bn_pcimg']['tmp_name']) ? $_FILES['it_bn_pcimg']['tmp_name'] : '';
$it_bn_pcimg_name	= isset($_FILES['it_bn_pcimg']['name']) ? $_FILES['it_bn_pcimg']['name'] : '';
$it_bn_moimg		= isset($_FILES['it_bn_moimg']['tmp_name']) ? $_FILES['it_bn_moimg']['tmp_name'] : '';
$it_bn_moimg_name	= isset($_FILES['it_bn_moimg']['name']) ? $_FILES['it_bn_moimg']['name'] : '';
$it_bn_pcimg_del	= (isset($_POST['it_bn_pcimg_del']) && $_POST['it_bn_pcimg_del']) ? preg_replace('/[^0-9]/', '', $_POST['it_bn_pcimg_del']) : 0;
$it_bn_moimg_del	= (isset($_POST['it_bn_moimg_del']) && $_POST['it_bn_moimg_del']) ? preg_replace('/[^0-9]/', '', $_POST['it_bn_moimg_del']) : 0;
$it_bn_url			= isset($_POST['it_bn_url']) ? strip_tags(clean_xss_attributes($it_bn_url)) : '';
//$it_bn_alt			= isset($_POST['it_bn_alt']) ? strip_tags(clean_xss_attributes($it_bn_alt)) : '';
//$it_bn_alt		= isset($_POST['it_bn_alt']) ? clean_xss_tags($_POST['it_bn_alt'], 1, 1) : '';
$it_bn_order		= isset($_POST['it_bn_order']) ? (int) $_POST['it_bn_order'] : 0;

if ($it_bn_pcimg_del)  @unlink(G5_DATA_PATH."/banner_item/pc_".$it_id);
if ($it_bn_moimg_del)  @unlink(G5_DATA_PATH."/banner_item/mo_".$it_id);

//파일이 이미지인지 체크합니다.
if( $it_bn_pcimg || $it_bn_pcimg_name ){
	if( !preg_match('/\.(gif|jpe?g|bmp|png)$/i', $it_bn_pcimg_name) ){
		alert("배너는 이미지 파일만 업로드 할수 있습니다.");
	}
	
	$timg = @getimagesize($it_bn_pcimg);
	if ($timg['2'] < 1 || $timg['2'] > 16){
		alert("배너는 이미지 파일만 업로드 할수 있습니다.");
	}
}

if( $it_bn_moimg || $it_bn_moimg_name ){
	if( !preg_match('/\.(gif|jpe?g|bmp|png)$/i', $it_bn_moimg_name) ){
		alert("배너는 이미지 파일만 업로드 할수 있습니다.");
	}
	
	$timg = @getimagesize($it_bn_moimg);
	if ($timg['2'] < 1 || $timg['2'] > 16){
		alert("배너는 이미지 파일만 업로드 할수 있습니다.");
	}
}
...
if ($w == "" || $w == "u")
{
	...
	@set_cookie("ck_weight", stripslashes($it_weight), time() + 86400*31);
	@set_cookie("ck_prescription", stripslashes($it_prescription), time() + 86400*31);
	@set_cookie("ck_takeway", stripslashes($it_takeway), time() + 86400*31);
	@set_cookie("ck_package", stripslashes($it_package), time() + 86400*31);
}
...
$it_subject = isset($_POST['it_subject']) ? strip_tags(clean_xss_attributes(trim($_POST['it_subject']))) : '';
$it_kind = (isset($_POST['it_kind']) && in_array($_POST['it_kind'], array('general', 'prescription'))) ? $_POST['it_kind'] : 'general';// 상품종류
...
if($it_mb_inf_cnt) {
    // 추가옵션명
    $arr_inf = array();
    for($i=0; $i<$it_mb_inf_cnt; $i++) {
        $post_inf_id = isset($_POST['it_mb_inf'][$i]) ? preg_replace(G5_OPTION_ID_FILTER, '', strip_tags($_POST['it_mb_inf'][$i])) : '';

        $inf_val = explode(chr(30), $post_inf_id);
        if(!in_array($inf_val[0], $arr_inf))
            $arr_inf[] = $inf_val[0];
    }

    $it_mb_inf = implode(',', $arr_inf);
}
...
'it_type4',             // 상품유형(인기)
'it_type5',             // 상품유형(할인)
'it_certification1',	// 기관()
'it_certification2',	// 기관()
'it_certification3',	// 기관()
'it_certification4',	// 기관()
'it_certification5',	// 기관()
'it_certification6',	// 기관()
'it_certification7',	// 기관()
'it_certification8',	// 기관()
'it_certification9',	// 기관()
'it_certification10',	// 기관()
'it_inf_price',	// 인플루언서 차감금액
//'it_mb_inf',	// 인플루언서 아이디
'it_bn_alt',	// 배너이미지설명
'it_bn_url',	// 배너링크
);
...
$it_shipping_process = isset($_POST['it_shipping_process']) ? $_POST['it_shipping_process'] : '';
$it_faq = isset($_POST['it_faq']) ? $_POST['it_faq'] : '';
$it_change_content = isset($_POST['it_change_content']) ? $_POST['it_change_content'] : '';
$it_baesong_content = isset($_POST['it_baesong_content']) ? $_POST['it_baesong_content'] : '';
...
$sql_common = " ca_id               = '$ca_id',
				...
				it_kind             = '$it_kind',
				it_weight			= '$it_weight',
				it_prescription		= '$it_prescription',
				it_takeway			= '$it_takeway',
                ...
                it_type4            = '$it_type4',
                it_type5            = '$it_type5',
				it_certification1	= '$it_certification1',
				it_certification2	= '$it_certification2',
				it_certification3	= '$it_certification3',
				it_certification4	= '$it_certification4',
				it_certification5	= '$it_certification5',
				it_certification6	= '$it_certification6',
				it_certification7	= '$it_certification7',
				it_certification8	= '$it_certification8',
				it_certification9	= '$it_certification9',
				it_certification10	= '$it_certification10',
				it_inf_price		= '$it_inf_price',
				it_mb_inf			= '$it_mb_inf',
                it_basic            = '$it_basic',
				it_precautions		= '$it_precautions',
				it_shipping_process	= '$it_shipping_process',
				it_faq				= '$it_faq',
				it_change_content	= '$it_change_content',
				it_baesong_content	= '$it_baesong_content',
                it_explan           = '$it_explan',
...
if ($w == "" || $w == "u")
{
	...
	if ($_FILES['it_bn_pcimg']['name']) upload_file($_FILES['it_bn_pcimg']['tmp_name'], 'pc_'.$it_id, G5_DATA_PATH."/banner_item");
	if ($_FILES['it_bn_moimg']['name']) upload_file($_FILES['it_bn_moimg']['tmp_name'], 'mo_'.$it_id, G5_DATA_PATH."/banner_item");
}
...
if(is_checked('chk_ca_it_subject'))				$ca_fields .= " , it_subject = '$it_subject' ";
if(is_checked('chk_ca_it_order'))               $ca_fields .= " , it_order = '$it_order' ";
if(is_checked('chk_ca_it_icon'))                $ca_fields .= " , it_icon1 = '$it_icon1', it_icon2 = '$it_icon2', it_icon3 = '$it_icon3', it_icon4 = '$it_icon4', it_icon5 = '$it_icon5' ";
...
if(is_checked('chk_ca_it_it_certification'))	$ca_fields .= " , it_certification1 = '$it_certification1', it_certification2 = '$it_certification2', it_certification3 = '$it_certification3', it_certification4 = '$it_certification4', it_certification5 = '$it_certification5', it_certification6 = '$it_certification6', it_certification7 = '$it_certification7', it_certification8 = '$it_certification8', it_certification9 = '$it_certification9', it_certification10 = '$it_certification10' ";


if(is_checked('chk_ca_it_maker'))               $ca_fields .= " , it_maker = '$it_maker' ";
if(is_checked('chk_ca_it_weight'))				$ca_fields .= " , it_weight = '$it_weight' ";
if(is_checked('chk_ca_it_prescription'))		$ca_fields .= " , it_prescription = '$it_prescription' ";
if(is_checked('chk_ca_it_takeway'))				$ca_fields .= " , it_takeway = '$it_takeway' ";
if(is_checked('chk_ca_it_package'))				$ca_fields .= " , it_package = '$it_package' ";
...
if(is_checked('chk_ca_it_inf_price'))			$ca_fields .= " , it_inf_price = '$it_inf_price' ";
if(is_checked('chk_ca_it_mb_inf'))               $ca_fields .= " , it_mb_inf = '$it_mb_inf' ";
...
if(is_checked('chk_all_it_subject'))			 $all_fields .= " , it_subject = '$it_subject' ";
if(is_checked('chk_all_it_basic'))               $all_fields .= " , it_basic = '$it_basic' ";
if(is_checked('chk_all_it_order'))               $all_fields .= " , it_order = '$it_order' ";
if(is_checked('chk_all_it_icon'))                $all_fields .= " , it_icon1 = '$it_icon1', it_icon2 = '$it_icon2', it_icon3 = '$it_icon3', it_icon4 = '$it_icon4', it_icon5 = '$it_icon5' ";
...
if(is_checked('chk_all_it_certification'))		 $all_fields .= " , it_certification1 = '$it_certification1', it_certification2 = '$it_certification2', it_certification3 = '$it_certification3', it_certification4 = '$it_certification4', it_certification5 = '$it_certification5', it_certification6 = '$it_certification6', it_certification7 = '$it_certification7', it_certification8 = '$it_certification8', it_certification9 = '$it_certification9', it_certification10 = '$it_certification10' ";
if(is_checked('chk_all_it_weight'))				 $all_fields .= " , it_weight = '$it_weight' ";
if(is_checked('chk_all_it_prescription'))		 $all_fields .= " , it_prescription = '$it_prescription' ";
if(is_checked('chk_all_it_takeway'))			 $all_fields .= " , it_takeway = '$it_takeway' ";
if(is_checked('chk_all_it_package'))			 $all_fields .= " , it_package = '$it_package' ";
...
if(is_checked('chk_all_it_inf_price'))			 $all_fields .= " , it_inf_price = '$it_inf_price' ";
if(is_checked('chk_all_it_mb_inf'))				 $all_fields .= " , it_mb_inf = '$it_mb_inf' ";
