...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
            <input type="file" name="bn_pcimg" id="bn_pcimg" accept="image/gif, image/jpeg, image/png">
            <?php
            $pcimg_str = "";
            $bimg = G5_DATA_PATH."/banner/pc_{$bn['bn_id']}";
            if (file_exists($bimg) && $bn['bn_id']) {
                ...
                echo '<input type="checkbox" name="bn_pcimg_del" value="1" id="bn_pcimg_del"> <label for="bn_pcimg_del">삭제</label>';
                $pcimg_str = '<img src="'.G5_DATA_URL.'/banner/pc_'.$bn['bn_id'].'" width="'.$width.'">';
            }
            if ($pcimg_str) {
	...
	<tr>
        ...
        <td>
            <input type="file" name="bn_moimg" id="bn_moimg" accept="image/gif, image/jpeg, image/png">
            <?php
            $moimg_str = "";
            $bimg = G5_DATA_PATH."/banner/mo_{$bn['bn_id']}";
            if (file_exists($bimg) && $bn['bn_id']) {
                $size = @getimagesize($bimg);
                if($size[0] && $size[0] > 750)
                    $width = 750;
                ...
                echo '<input type="checkbox" name="bn_moimg_del" value="1" id="bn_moimg_del"> <label for="bn_moimg_del">삭제</label>';
                $moimg_str = '<img src="'.G5_DATA_URL.'/banner/mo_'.$bn['bn_id'].'" width="'.$width.'">';
            }
            if ($moimg_str) {
                echo '<div class="banner_or_img">';
                echo $moimg_str;
	...
	<!--
	<tr>
        <th scope="row"><label for="bn_subject">배너제목</label></th>
        <td>
            <?php echo help("탭방식 배너일 경우 탭이름."); ?>
            <input type="text" name="bn_subject" size="80" value="<?php echo get_text($bn['bn_subject']); ?>" id="bn_subject" class="frm_input">
        </td>
    </tr>
	-->
    <tr>
        ...
        <td>
			...
			<!--<textarea name="bn_alt" id="bn_alt" class="textarea" style="width:100%;height:50px"><?php echo get_text($bn['bn_alt']); ?></textarea>-->
    ...
    <tr>
        ...
        <td>
            <select name="bn_position" id="bn_position">
				<?php foreach($_const['banner_position'] as $key => $value) { ?>
				<option value="<?php echo $key ?>"<?php echo get_selected($key, $bn['bn_position']) ?>><?php echo $value ?></option>
				<?php } ?>
			</select>
        </td>
    </tr>
	<tr>
        <th scope="row"><label for="bn_position">출력스킨</label></th>
        <td>
			<select name="bn_skin" id="bn_skin">
				<?php echo get_list_skin_options("^banner.[0-9a-z_]+\.skin\.php", G5_SKIN_PATH.'/banner', $bn['bn_skin']); ?>
			</select>
        </td>
    </tr>
    <tr>
