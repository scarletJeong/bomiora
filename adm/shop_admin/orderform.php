...
<section id="anc_sodr_list">
    ...
    <div class="tbl_head01 tbl_wrap">
        <table>
        <caption>주문 제품 목록</caption>
        <thead>
        <tr>
            <th scope="col">제품명</th>
            <th scope="col">
                <label for="sit_select_all" class="sound_only">주문 제품 전체</label>
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
			...
			// 처방전 여부
			$sql = " select * from {$g5['g5_shop_health_profile_cart_table']} where it_id = '{$row['it_id']}' and od_id = '$od_id' and hp_output = 'Y' ";
			$prescription = sql_fetch($sql);
			$prescription_status = false;
			if($prescription) {
				$arr_it_id[] = $row['it_id'];
				$arr_it_name[$row['it_id']] = $row['it_name'];
            ...
            <tr>
                ...
                <td rowspan="<?php echo $rowspan; ?>" class="td_left">
                    ...
                    <?php if($od['od_tax_flag'] && $row['ct_notax']) echo '[비과세제품]'; ?>
				<?php echo ($prescription_status) ? '(처방제품)' : ''; ?>
                </td>
                ...
                <td class=" td_num_right"><?php echo number_format($opt['ct_point']); ?></td>
...
<!-- 처방 -->
<section id="anc_sodr_health">
	<h2 class="h2_frm">처방정보</h2>
    <?php echo $pg_anchor; ?>
	<!--
    <div class="local_desc02 local_desc">
        <p>
	...
	<div class="tbl_head01 tbl_wrap">
		<table>
			...
			<tbody>
				<?php
				// 제품이 여러개일 경우 하나만 나오게 할 수 없다.
				if(is_array($arr_it_id) && !empty($arr_it_id)) {
					$unq_it_id = array_unique($arr_it_id);
					foreach($unq_it_id as $it_id) {
						// 중복체크
						$sql = " select * from {$g5['g5_shop_health_profile_cart_table']} where mb_id = '{$od['mb_id']}' and od_id = '{$od_id}' and it_id = '{$it_id}' and hp_output = 'Y' ";
						//echo $sql."<br>";
						$dc = sql_fetch($sql);
						if(!$dc) continue;
						$bg = 'bg'.($i%2);
				...
				<tr class="<?php echo $bg; ?>">
					<td><?php echo $arr_it_name[$it_id] ?></td>
					<td><?php echo $dc['mb_id'] ?></td>
					<td><?php echo get_text($dc['hp_rsvt_name']) ?></td>
					<td><?php echo get_text($dc['answer_1']) ?></td>
					<td><?php echo $_const['sex'][$dc['answer_2']] ?></td>
					<td><?php echo number_format($dc['answer_3']) ?>KG</td>
					<td><?php echo number_format($dc['answer_4']) ?>CM</td>
					<td><?php echo number_format($dc['answer_5']) ?>KG</td>
					<td><?php echo $_const['diet_period'][$dc['answer_6']] ?></td>
					<td><?php echo date("Y.m.d", strtotime($dc['hp_rsvt_date'])) ?> <?php echo get_yoil($dc['hp_rsvt_date']) ?></td>
					<td><?php echo get_text($dc['hp_rsvt_stime']) ?> ~ <?php echo get_text($dc['hp_rsvt_etime']) ?></td>
					<td class="td_mng td_mns_m">
						<a href="./orderform_health.php?od_id=<?php echo $od_id; ?>&amp;hp_no=<?php echo $dc['hp_no']; ?>&amp;<?php echo $qstr; ?>" class="btn btn_03">수정</a>
					</td>
				</tr>
				<?php
					}// end foreach
...
function form_submit(f)
{
    ...
    <?php if($od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == 'KAKAOPAY' || $od['od_settle_case'] == '간편결제'
     || ($od['od_pg'] == 'kcp' && ($od['od_settle_case'] == '계좌이체' || $od['od_settle_case'] == '가상계좌'))
     || ($od['od_pg'] == 'inicis' && is_inicis_order_pay($od['od_settle_case']) )) { ?>
    ...
    if (confirm(msg+"\'" + status + "\' 상태를 선택하셨습니다.\n\n선택하신대로 처리하시겠습니까?")) {
      ...
      if (f.pg_cancel.value) {
      <?php if($od['od_settle_case'] == '가상계좌') { ?>
        alert("가상계좌 결제 취소의 경우에는 직접 환불을 진행하여야 합니다.");
      <?php } ?>
      }
