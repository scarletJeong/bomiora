...
//$mb_id			= isset($_POST['mb_id']) ? get_email_address(trim($_POST['mb_id'])) : '';
...
// 아이디 생성
if ($w == '') {
	$in_mb = sql_fetch("select count(*) as in_cnt from {$g5['member_table']} where mb_level = '{$_const['level']['인플루언서']}' ");
	$mb_id = 'influencer'.($in_mb['in_cnt'] + 1);
} elseif ($w == 'u') {
	$mb = get_member_email($mb_id);// 아이디(이메일)/비밀번호 로그인
	$mb_id = $mb['mb_id'];
}
