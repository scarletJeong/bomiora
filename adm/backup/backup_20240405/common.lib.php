    ...
    if( !$src ){
		if($member['profile_img']) {
			...
			if( !empty($no_profile_cache) ){
				...
				$no_profile_cache = $src = isset($tmp[1]) ? $tmp[1] : G5_IMG_URL.'/profile/profile_img_01.png';
			}
		}
    }
...
//알림톡 수정 jacknam
function bizm_alimtalk($receive_number, $profile, $tmplId, $msg, $userId, $title = '', $extraJson = '') {
	if(!$receive_number || !$profile || !$tmplId || !$msg || !$userId) return false;

	$message_type = 'at';//알림톡
	$phn = '82'.substr($receive_number, 1);// 사용자 전화번호

	if ($title) { // 강조형 제목
	  $title = ',"title":"'.$title.'"';
	}

	$post_fields = '[{"message_type":"'.$message_type.'","profile":"'.$profile.'","phn":"'.$phn.'","tmplId":"'.$tmplId.'","msg":"'.$msg.'"'.$title.$extraJson.'}]';
	//return $post_fields;
...
function get_od_invoice_url($od_delivery_company, $od_invoice) {
  ...
  for ($i=0; $i < count($dlcomp); $i++) {
    ...
    if($od_delivery_company == $value) {
      $od_invoice_url = $url . str_replace('-', '', $od_invoice);
      break;
    }
  }
  return $od_invoice_url;
}

function get_od_talkdata($od_id) {
  global $g5;

  $sql = "SELECT d.*, COUNT(1) AS 'count' FROM (
SELECT a.od_id,od_name,od_hp,it_name,od_delivery_company,od_invoice,ct_option,hp_rsvt_date,hp_rsvt_stime,hp_rsvt_etime,hp_rsvt_name,hp_rsvt_tel
FROM {$g5['g5_shop_order_table']} a
LEFT JOIN {$g5['g5_shop_cart_table']} b ON a.od_id = b.od_id
LEFT JOIN {$g5['g5_shop_health_profile_cart_table']} c ON a.od_id = c.od_id
WHERE a.od_id = '{$od_id}' GROUP BY b.it_name) d GROUP BY d.od_id LIMIT 1;";
  ...
  if ($od['hp_rsvt_date'] && $od['hp_rsvt_stime'] && $od['hp_rsvt_etime']) {
    $od['od_rsvt_date'] = $od['hp_rsvt_date'].' '.$od['hp_rsvt_stime'].'~'.$od['hp_rsvt_etime'];
  }

  if ($od['count']) {
    $od['count'] = (int)$od['count'];
  }

  if ($od['od_delivery_company'] && $od['od_invoice']) {
    $od['od_invoice_url'] = get_od_invoice_url($od['od_delivery_company'], $od['od_invoice']);
  }

  if (!$od['od_name'] && $od['hp_rsvt_name']) {
    $od['od_name'] = $od['hp_rsvt_name'];
  }

  if (!$od['od_hp'] && $od['hp_rsvt_tel']) {
    $od['od_hp'] = $od['hp_rsvt_tel'];
  }

  if (!$od['hp_rsvt_name'] && $od['od_name']) {
    $od['hp_rsvt_name'] = $od['od_name'];
  }

  if (!$od['hp_rsvt_tel'] && $od['od_hp']) {
    $od['hp_rsvt_tel'] = $od['od_hp'];
  }

  return $od;
}

function get_cp_talkdata($cp_id) {
  global $g5;

  $sql = "select a.cp_id,a.cp_subject,a.cp_method,a.cp_target,a.mb_id,a.cp_start,a.cp_end,a.cp_price,a.cp_type,a.cp_maximum,
  b.mb_name,b.mb_nick,b.mb_tel,b.mb_hp,b.mb_email,c.it_name,d.ca_name
  from {$g5['g5_shop_coupon_table']} a left join {$g5['member_table']} b on a.mb_id = b.mb_id
  left join {$g5['g5_shop_item_table']} c on a.cp_target = c.it_id left join bomiora_shop_category d on a.cp_target = d.ca_id
  where a.cp_id = '{$cp_id}' limit 1;";

  //return $sql;

  $cp = sql_fetch($sql);
  if(!$cp['cp_id'] || !$cp['cp_subject']) {
    return null;
  }

  $mb_name = $cp['mb_name'] ? $cp['mb_name'] : $cp['mb_nick'];
  if (!$mb_name) {
    return null;
  }
  ...
  if ($cp['cp_method'] == '0') {
    $cp_item = $cp['it_name'];
  } else if ($cp['cp_method'] == '1') {
    $cp_item = $cp['ca_name'] ? $cp['ca_name'] . '군' : '';
  } else if ($cp['cp_method'] == '2') {
    $cp_item = '모든 상품';
  } else if ($cp['cp_method'] == '3') {
    $cp_item = '배송비 할인';
  }
  if (!$cp_item) {
    return null;
  ...
  } else if ($cp['cp_type'] == '1' && (int)$cp['cp_price'] < 100) {
    $cp_price = '상품가격의 ' . $cp['cp_price'] . '%';
    if ($cp['cp_maximum']) {
      $cp_price .= ' (최대 ' . number_format($cp['cp_maximum']) . '원)';
    }
  }
  if (!$cp_price) {
    return null;
  ...
  $cp_sdate = $cp['cp_start'] ? $cp['cp_start'] : '제한없음';
  if ($cp['cp_end']) {
    if (strtotime(date('Y-m-d H:i:s')) < strtotime($cp['cp_end'] . ' 23:59:59')) {
      $cp_edate = $cp['cp_end'];
    }
  } else {
    $cp_edate = '제한없음';
  ...
  $cp_hp = hyphen_hp_number($cp['mb_hp'], true);
  if (!$cp_hp) {
    $cp_hp = hyphen_hp_number($cp['mb_tel'], true);
    if (!$cp_hp) {
      return null;
    }
  }

  $talk_data = array('cp_id' => $cp['cp_id'], 'cp_name' => $cp['cp_subject'], 'cp_mb_name' => $mb_name, 'cp_hp' => $cp_hp, 'cp_email' => $cp['mb_email'],
  'cp_item' => $cp_item, 'cp_price' => $cp_price, 'cp_sdate' => $cp_sdate, 'cp_edate' => $cp_edate);

  return $talk_data;
}

function order_alimtalk($target_action, $od_id, $extra_numbers = []) {
  $results = [];
  $tmp_title = '';
  $tmp_msg	= '';
  $extra_json = '';
  $profiles = array('clinic'=>'33a18c983224c66c1f640b150bf048b344c32129', 'corp'=>'48f08964f8b00bdc1f5e401b9b5abfbcff6bbfe5');

  $talk_data = get_od_talkdata($od_id);
  if (!$talk_data['od_hp'] || !$talk_data['it_name'] || !$talk_data['count']) {
    return $results;
  }

  switch ($target_action) {
    case 'payment':
    if (!$talk_data['od_id'] || !$talk_data['od_rsvt_date']) {
      return $results;
    }
    $profile = 'clinic';
    //$tmp_id = 'de_sms_use8';
    //$tmp_title = '결제 완료';
    //$tmp_msg = '안녕하세요, #{이름} 고객님.\n\n#{제품명}[#{주문번호}] 처방이 #{주문날짜}에 결제 완료되었습니다.\n\n#{진료시간}에 비대면진료 전화가 올 예정입니다.\n\n감사합니다.';
    $tmp_id = 'bomiora_1';
    $tmp_title = '결제, 예약 완료';
    $tmp_msg = '안녕하세요, #{이름} 고객님.\n\n#{제품명}[#{주문번호}] 처방이 #{주문날짜}에 결제 완료되었습니다.\n\n#{진료시간}에 원장님의 비대면진료 전화가 올 예정입니다.\n\n감사합니다.\n\n■보미오라한의원 강남본점\n■전화번호 : 02-546-1031';    
    $tmp_msg = str_replace("#{주문번호}", $talk_data['od_id'], $tmp_msg);
    $tmp_msg = str_replace("#{주문날짜}", date('Y-m-d H:i', G5_SERVER_TIME), $tmp_msg);
    $tmp_msg = str_replace("#{진료시간}", $talk_data['od_rsvt_date'], $tmp_msg);
    $extra_json = ',"button1":{"name":"채널 추가","type":"AC"}';
    $extra_json .= ',"button2":{"name":"보미오라한의원","type":"WL","url_mobile":"http://bomiora.com","url_pc":"http://bomiora.com"}';
    $extra_json .= ',"button3":{"name":"보미오라몰","type":"WL","url_mobile":"http://bomiora.kr","url_pc":"http://bomiora.kr"}';
    break;
    case 'prescription':
    $profile = 'clinic';
    $tmp_id = 'de_sms_use9';
    $tmp_title = '처방 완료';
    $tmp_msg = '안녕하세요, #{이름} 고객님.\n\n#{제품명} 처방이 #{처방날짜}일 완료되었습니다.\n\n감사합니다.';
    $tmp_msg = str_replace("#{처방날짜}", date('Y년 m월 d', G5_SERVER_TIME), $tmp_msg);
    if ($talk_data['hp_rsvt_tel'] && ($talk_data['hp_rsvt_tel'] != $talk_data['od_hp'])) {
      $talk_data['od_hp'] = $talk_data['hp_rsvt_tel'];
    }
    break;
    case 'delivery':
    if (!$talk_data['od_delivery_company'] || !$talk_data['od_invoice'] || !$talk_data['od_invoice_url']) {
      return $results;
    }
    $profile = 'clinic';
    $tmp_id = 'de_sms_use10';
    $tmp_title = '배송 준비';
    $tmp_msg = '안녕하세요, #{이름} 고객님.\n\n신청하신 #{제품명}의 배송 준비를 시작합니다.\n처방 완료 후 출고될 예정입니다.\n\n■택배사 : #{택배회사}\n■운송장번호 : #{운송장번호}\n■배송조회 : #{배송조회링크}\n\n감사합니다.';
    $tmp_msg = str_replace("#{택배회사}", $talk_data['od_delivery_company'], $tmp_msg);
    $tmp_msg = str_replace("#{운송장번호}", $talk_data['od_invoice'], $tmp_msg);
    $tmp_msg = str_replace("#{배송조회링크}", $talk_data['od_invoice_url'], $tmp_msg);
    break;
    case 'reservation':
    if (!$extra_numbers || !$talk_data['od_rsvt_date']) {
      return $results;
    }
    $profile = 'clinic';
    $tmp_id = 'doctor_2';
    $tmp_title = '결제, 예약 완료';
    $tmp_msg = '■고객명 : #{이름}님\n■연락처 : #{연락처}\n■제품명 : #{제품명}\n■예약 : #{진료시간}';
    $tmp_msg = str_replace("#{연락처}", $talk_data['od_hp'], $tmp_msg);
    $tmp_msg = str_replace("#{진료시간}", $talk_data['od_rsvt_date'], $tmp_msg);
    break;
    default:
    return $results;
  ...
  if($talk_data['count'] > 1) {
    $od_count = $talk_data['count'] - 1;
    $od_it_name = $talk_data['it_name'].' 외 '.$od_count.'건';
  } else {
    $od_it_name = $talk_data['it_name'];
  }

  if ($talk_data['ct_option']) {
    $od_it_name .= ' | ' . $talk_data['ct_option'];
  }

  $tmp_msg = str_replace("#{제품명}", $od_it_name, $tmp_msg);
  $tmp_msg = str_replace("#{이름}", $talk_data['od_name'], $tmp_msg);

  $profile = $profiles[$profile];
  $user_id = 'bomioramall01';

  //$default['de_sms_hp']
  $recv_numbers	= $target_action == 'reservation' ? [] : array($talk_data['od_hp']);
  foreach($extra_numbers as $number) {
    $recv_numbers[] = $number;
  }

  for ($i = 0; $i < count($recv_numbers); $i++) {
    $recv_number = preg_replace("/[^0-9]/", "", $recv_numbers[$i]);
    $results[] = bizm_alimtalk($recv_number, $profile, $tmp_id, $tmp_msg, $user_id, $tmp_title, $extra_json);
  }

  return $results;
}

function coupon_alimtalk($target_action, $cp_id, $send_alimtalk = true) {
  $results = [];
  $tmp_title = '';
  $tmp_msg	= '';
  $extra_json = '';
  //$now_date = date('Y-m-d H:i', G5_SERVER_TIME);
  $profiles = array('clinic'=>'33a18c983224c66c1f640b150bf048b344c32129', 'corp'=>'48f08964f8b00bdc1f5e401b9b5abfbcff6bbfe5');

  $talk_data = get_cp_talkdata($cp_id);
  if (!$talk_data['cp_id'] || !$talk_data['cp_hp'] || !$talk_data['cp_name'] || !$talk_data['cp_mb_name']) {
    return $results;
  }

  switch ($target_action) {
    case 'coupon1':
    if (!$talk_data['cp_item'] || !$talk_data['cp_price'] || !$talk_data['cp_sdate'] || !$talk_data['cp_edate']) {
      return $results;
    }
    $profile = 'clinic';
    $tmp_id = 'coupon_1';
    $tmp_title = '쿠폰 발급 안내';
    $tmp_msg = '안녕하세요, #{이름} 고객님.\n\n보미오라 할인 쿠폰이 발급되었습니다♥\n(마이페이지에서 쿠폰/포인트 확인 가능)\n\n할인이 적용된 링크 외에서 \n자유롭게 쿠폰 사용 가능하십니다.\n\n보미오라 고객님들의 다이어트 성공을 기원합니다. \n\n감사합니다:)\n\n■쿠폰이름 : #{쿠폰이름}\n■적용상품 :  #{적용상품}\n■할인금액 :  #{할인금액}\n■사용 시작일 : #{사용시작일}\n■사용 종료일 : #{사용종료일}\n\n- 이 메시지는 이용약관(계약서) 동의에 따라 지급된 쿠폰 안내 메시지입니다.';
    $tmp_msg = str_replace("#{적용상품}", $talk_data['cp_item'], $tmp_msg);
    $tmp_msg = str_replace("#{할인금액}", $talk_data['cp_price'], $tmp_msg);
    $tmp_msg = str_replace("#{사용시작일}", $talk_data['cp_sdate'], $tmp_msg);
    $tmp_msg = str_replace("#{사용종료일}", $talk_data['cp_edate'], $tmp_msg);
    $extra_json = ',"button1":{"name":"보미 다이어트환 처방 신청","type":"WL","url_mobile":"https://bomiora.kr/shop/item.php?it_id=1686290723","url_pc":"https://bomiora.kr/shop/item.php?it_id=1686290723"}';
    $extra_json .= ',"button2":{"name":"보미 디톡스환 처방 신청","type":"WL","url_mobile":"https://bomiora.kr/shop/item.php?it_id=1682401752","url_pc":"https://bomiora.kr/shop/item.php?it_id=1682401752"}';
    $extra_json .= ',"button3":{"name":"보미오라몰","type":"WL","url_mobile":"https://bomiora.kr","url_pc":"https://bomiora.kr"}';
    $extra_json .= ',"button4":{"name":"보미오라한의원","type":"WL","url_mobile":"http://bomiora.com","url_pc":"http://bomiora.com"}';
    break;
    default:
    return $results;
    break;
  ...
  $tmp_msg = str_replace("#{쿠폰이름}", $talk_data['cp_name'], $tmp_msg);
  $tmp_msg = str_replace("#{이름}", $talk_data['cp_mb_name'], $tmp_msg);

  $profile = $profiles[$profile];
  $user_id = 'bomioramall01';

  $results['title'] = $tmp_title;
  $results['contents'] = $tmp_msg;
  $results['link'] = $extra_json;
  $results['mb_name'] = $talk_data['cp_mb_name'];
  $results['mb_email'] = $talk_data['cp_email'];

  if ($send_alimtalk) {
    $recv_number	= preg_replace("/[^0-9]/", "", $talk_data['cp_hp']);
    if ($recv_number) {
      $results['mb_hp'] = $recv_number;
      $results['alimtalk_result'] = bizm_alimtalk($recv_number, $profile, $tmp_id, $tmp_msg, $user_id, $tmp_title, $extra_json);
    }
  }
...
function resize_image($filename, $max_width, $max_height) {
  ...
  return array('data' => $image_p, 'type' => $type, 'width' => $width, 'height' => $height);
}

function get_popup($pu_location) {
  global $g5;
  
  $is_mobile = is_mobile();

  $pu_list = [];
  $sql = "SELECT * FROM {$g5['g5_shop_popup_table']} WHERE pu_location = '{$pu_location}' AND pu_device <> '" . ($is_mobile ? 'pc' : 'mobile')
  . "' AND pu_end_time > '" . G5_TIME_YMDHIS . "' ORDER BY pu_order DESC, pu_id DESC LIMIT 5;";

  $img_types = ['jpg', 'png', 'gif'];
  $result = sql_query($sql);
  if ($result) {
    for ($i = 0; $row = sql_fetch_array($result); $i++) {
      if (in_array($row['pu_type'], $img_types)) {
        $file_path = "/popup/popup_{$row['pu_id']}.{$row['pu_type']}";
        if (file_exists(G5_IMG_PATH . $file_path)) {
          $row['pu_img'] = G5_IMG_URL . $file_path;
        } else {
          continue;
        }
      }
      if (!$row['pu_url']) $row['pu_url'] = '#';
      $pu_list[] = $row;
    }
  }
