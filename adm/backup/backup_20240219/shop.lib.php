...
class item_list
{
	...
	protected $view_it_subject  = true;        // 제목
...
function get_it_image($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=false)
{
    ...
    if($thumb) {
        ...
        $img = '<img src="'.$file_url.'" alt="'.$img_alt.'"';// width="'.$width.'" height="'.$height.'"
    } else {
        //$img = '<img src="'.G5_SHOP_URL.'/img/no_image.gif" width="'.$width.'"';
		$img = '<img src="'.G5_IMG_URL.'/detail/poto_review.png" width="'.$width.'"';
        if($height)
            $img .= ' height="'.$height.'"';
        $img .= ' alt="'.$img_alt.'"';
    }
...
// 상품이미지 썸네일 생성 :::: 수정(psk)
function get_it_thumbnail($img, $width, $height=0, $id='', $is_crop=false, $is_size=false)
{
    $str = '';
    ...
    if($subj_count > 1) {
        ...
        for($i=0; $i<$subj_count; $i++) {
            ...
            if($opt_count) {
                ...
                if($is_div === 'div') {
                    $str .= '<div class="get_item_options option_text">'.PHP_EOL;
...
function get_item_supply($it_id, $subject, $is_div='', $is_first_option_title='')
{
    ...
    for($i=0; $i<$subj_count; $i++) {
        ...
        if($opt_count) {
            $seq = $i + 1;
            if($is_div === 'div') {
                $str .= '<div class="get_item_supply option_text">'.PHP_EOL;
...
// 배너출력 :: 수정(psk)
function display_banner($position, $random = 'n')
{
	global $g5;
    ...
    $sql = " select bn_id, bn_skin from {$g5['g5_shop_banner_table']} where '".G5_TIME_YMDHIS."' between bn_begin_time and bn_end_time $sql_device and bn_position = '$position' limit 1 ";
	$bn = sql_fetch($sql);

	$skin_path = G5_SKIN_PATH.'/banner/'.$bn['bn_skin'];
	if(G5_IS_MOBILE)
		$skin_path = G5_MOBILE_PATH.'/'.G5_SKIN_DIR.'/'.$bn['bn_skin'];

	if(file_exists($skin_path)) {
		// 배너 출력
		if($random =='y') {
			$sql = " select * from {$g5['g5_shop_banner_table']} where '".G5_TIME_YMDHIS."' between bn_begin_time and bn_end_time $sql_device and bn_position = '$position' order by rand() ";
		} else {
...
*/
	if ($it['it_icon1'])
        $icon .= '<span class="bg_color_33">한의약품</span>';

	if ($it['it_icon2'])
        $icon .= '<span class="bg_color_EB6890">오늘의 배송</span>';
...
function get_item_sendcost($it_id, $price, $qty, $cart_id)
{
    ...
    $sql = " select it_id, ct_price, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty
                from {$g5['g5_shop_cart_table']}
                where it_id = '$it_id'
                  and od_id = '$cart_id'
                order by ct_id
...
// 사용후기의 선호도(별) 평균을 상품테이블에 저장합니다.:: 수정(psk)
function update_use_avg($it_id)
{
    global $g5;
    $row = sql_fetch(" select count(*) as cnt, sum(is_score1 + is_score2 + is_score3 + is_score4) as total from {$g5['g5_shop_item_use_table']} where it_id = '{$it_id}' and is_confirm = 1 ");
    $average = ($row['total'] && $row['cnt']) ? ($row['total'] / $row['cnt']) / 4 : 0;
...
function is_coupon_downloaded($mb_id, $cz_id)
{
    ...
    $sql = " select count(*) as cnt from {$g5['g5_shop_coupon_table']} where mb_id = '$mb_id' and cz_id = '$cz_id' ";// and cp_activation = 'Y'
    $row = sql_fetch($sql);
...
// jacknam
function get_point_share($arr_price, $point) {
  $sum = array_sum($arr_price);
  $last = count($arr_price) - 1;
  $r = [];
  $t = 0;
  foreach ($arr_price as $i => $c){
    $d = floor($c / $sum * $point / 100) * 100;
    $t += $d;
    if ($i == $last) {
      $r[] = $point - ($t - $d);
    } else {
...
// 사용 가능 쿠폰 리스트 jacknam
function get_coupon_infos($cp_ids) {
  $available_list = [];
  if (!$cp_ids) {
  ...
  if (is_array($cp_ids)) {
    $cp_id_cond = " IN ('" . implode("','", $cp_ids) . "')";
  } else {
    $cp_id_cond = " = '" . $cp_ids . "'";
  }

  $sql = " SELECT a.* FROM {$g5['g5_shop_coupon_table']} a LEFT JOIN {$g5['g5_shop_coupon_log_table']} b ON a.cp_id = b.cp_id
  WHERE a.cp_id {$cp_id_cond} AND cp_start <= '".G5_TIME_YMD."' AND cp_end >= '".G5_TIME_YMD."' AND (a.mb_id = '전체회원' or b.cl_id IS NULL) GROUP BY a.cp_id ORDER BY cp_no ";
...
function get_available_coupons($mb_id, $it_id, $ca_id) {
  $available_list = [];
  if (!$mb_id || !$it_id || !$ca_id) {
    return $available_list;
  }
  ...
  if (is_array($it_id)) {
    $cp_target_it = " IN ('" . implode("','", $it_id) . "')";
  } else {
    $cp_target_it = " = '{$it_id}'";
  }

  if (is_array($ca_id)) {
    $cp_target_ca = " IN ('" . implode("','", $ca_id) . "')";
  } else {
    $cp_target_ca = " = '{$ca_id}'";
  }

  $sql = " SELECT a.* FROM {$g5['g5_shop_coupon_table']} a
  LEFT JOIN {$g5['g5_shop_coupon_log_table']} b ON a.cp_id = b.cp_id AND a.mb_id = b.mb_id
  WHERE a.mb_id IN ( '{$mb_id}', '전체회원' )
  AND ((cp_method = '0' and cp_target {$cp_target_it}) OR (cp_method = '1' and cp_target {$cp_target_ca}) OR (cp_method IN ('2', '3')))
  AND cp_start <= '".G5_TIME_YMD."' AND cp_end >= '".G5_TIME_YMD."'
  AND b.cl_id IS NULL ORDER BY cp_method, cp_no ";
...
function get_cart_list($mb_id, $od_id, $stage, $remove_no_hp = false) {
  $cart_list = [];
  if (!$mb_id || !$od_id) {
    return $cart_list;
  }
  ...
  if ($stage == 'payment') {
    $cond = "and a.ct_select = '1'";
  } else {
    $cond = "";
  }

  $sql = " select a.ct_id,a.it_id,a.it_name,a.ct_price,a.ct_point,a.ct_option,a.ct_qty,a.io_price,a.ct_status,a.ct_send_cost,a.it_sc_type,a.inf_code,a.ct_output,a.ct_kind,a.ct_select,
  b.it_kind,b.ca_id,b.ca_id2,b.ca_id3,b.it_icon1,b.it_icon2,b.it_org_id,b.it_notax,b.it_mb_inf,c.ca_id as it_org_ca_id,c.it_kind,d.hp_no,d.hp_doc_name,d.hp_rsvt_date,d.hp_rsvt_stime,d.hp_rsvt_etime
  from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on a.it_id = b.it_id left join {$g5['g5_shop_item_table']} c on b.it_org_id = c.it_id
  left join {$g5['g5_shop_health_profile_cart_table']} d on (a.od_id = d.od_id and a.it_id = d.it_id) where a.mb_id = '{$mb_id}' and a.od_id = '{$od_id}' and a.ct_output = 'Y' {$cond} ";
  $sql .= " group by a.it_id, a.io_id order by a.ct_id ";

  $remove_ct_ids = [];
  $remove_hp_nos = [];
  $str_1h_after = strtotime(date('Y-m-d H:i:s'));// + 3600;
  $result = sql_query($sql);
  for ($i=0; $row=sql_fetch_array($result); $i++) {
    if ($remove_no_hp && $row['it_kind' == 'prescription']) {
      if (!$row['hp_no']) {
        $remove_ct_ids[] = $row['ct_id'];
        continue;
      } else {
        $str_rsvt = strtotime("{$row['hp_rsvt_date']} {$row['hp_rsvt_stime']}:00");
        if ($str_rsvt < $str_1h_after) {
          $remove_ct_ids[] = $row['ct_id'];
          $remove_hp_nos[] = $row['hp_no'];
          continue;
        }
      }
    }
  ...
  if ($remove_ct_ids) {
    $remove_ct_ids = implode(',', $remove_ct_ids);
    $sql = " delete from {$g5['g5_shop_cart_table']} where mb_id = '{$mb_id}' and od_id = '{$od_id}' and ct_id IN ({$remove_ct_ids}) ";
    sql_query($sql);
  }

  if ($remove_hp_nos) {
    $remove_hp_nos = implode(',', $remove_hp_nos);
    $sql = " delete from {$g5['g5_shop_health_profile_cart_table']} where mb_id = '{$mb_id}' and od_id = '{$od_id}' and hp_no IN ({$remove_hp_nos}) ";
    sql_query($sql);
  }
...
function is_coupon_point_disable($cart_list, $target = 'all') {
  ...
  $coupon_point_disable = false;
  foreach($cart_list as $cart) {
    if ($cart['it_mb_inf']) {
      $coupon_point_disable = true;
      break;
    }
  }

  if ($target == 'point' && !$coupon_point_disable) {
    global $config;
    if (!$config['cf_use_point']) {
      $coupon_point_disable = true;
    }
  ...
  return $coupon_point_disable;
}

function get_rsvt_info($cart_list, $buffer_sec = 0, $update = false) {
  if (!$cart_list) return false;
  ...
  $str_1h_after = strtotime(date('Y-m-d H:i:s')) - $buffer_sec;
  $rsvt_info = [];
  $update_infos = [];
  $insert_infos = [];
  $latest_rsvt;
  ...
  foreach($cart_list as $cart) {
    if ($cart['it_kind'] != 'prescription') {
      continue;
    }

    if (!$cart['hp_no']) {
      $insert_infos[] = ['od_id' => $cart['od_id'], 'it_id' => $cart['it_id']];
      continue;
    }

    $str_rsvt = strtotime("{$cart['hp_rsvt_date']} {$cart['hp_rsvt_stime']}:00");
    $update_infos[] = ['hp_no' => $cart['hp_no'], 'str_rsvt' => $str_rsvt];
    if ($str_rsvt < $str_1h_after) {
      continue;
    }

    if (!$latest_rsvt || $str_rsvt > $latest_rsvt) {
      $latest_rsvt = $str_rsvt;
      foreach (['hp_no', 'hp_doc_name', 'hp_rsvt_date', 'hp_rsvt_stime', 'hp_rsvt_etime'] as $key) {
        $rsvt_info[$key] = $cart[$key];
      }
    }
  }

  if ($update && $latest_rsvt) {
    $update_hp_nos = [];
    foreach($update_infos as $update) {
      if ($update['str_rsvt'] < $latest_rsvt) {
        $update_hp_nos[] = $update['hp_no'];
      }
    }

    if ($update_hp_nos) {
      global $g5;
      $update_hp_nos = implode(',', $update_hp_nos);
      $sql = " update {$g5['g5_shop_health_profile_cart_table']} set hp_doc_name = '{$rsvt_info['hp_doc_name']}'
      ,hp_rsvt_date = '{$rsvt_info['hp_rsvt_date']}',hp_rsvt_stime = '{$rsvt_info['hp_rsvt_stime']}',hp_rsvt_etime = '{$rsvt_info['hp_rsvt_etime']}'
      where hp_no in ({$update_hp_nos}) ";
      sql_query($sql);
    }
  }

  if ($rsvt_info) {
    $rsvt_info['hp_doc_name'] = get_text($rsvt_info['hp_doc_name']);
    $rsvt_info['hp_rsvt_date_yoil'] = date("Y.m.d", strtotime($rsvt_info['hp_rsvt_date'])) . ' ' . get_yoil($rsvt_info['hp_rsvt_date']) . '요일';
    $rsvt_info['hp_rsvt_time'] = get_text($rsvt_info['hp_rsvt_stime']) . ' ~ ' . get_text($rsvt_info['hp_rsvt_etime']);
  }
