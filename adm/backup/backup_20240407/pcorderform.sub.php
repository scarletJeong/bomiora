...
<div class="">
  ...
  <input type="hidden" name="hp_rsvt_ts" value="<?php echo strtotime($rsvt_info['hp_rsvt_date'] . ' ' . $rsvt_info['hp_rsvt_stime'] . ':00'); ?>" />
...
$(function() {
  ...
  /*
  $(document).on("click", ".payment_method_list input[name=od_settle_case]", function(e) {
    //e.preventDefault();

    var _this = $(this),
    id = _this.attr("id");

    if (id == "od_settle_bank") {
    ...
    if (id == "od_settle_iche" || id == "od_settle_vbank") {
      $("#escrow_select").show();
      $("#escrow_select input[type=radio]:last").prop("checked", "true");
    } else {
      $("#escrow_select").hide();
    }
  });
...
function format_datetime(dt, arr) {
  let datePart = [
  dt.getFullYear(),
  dt.getMonth() + 1,
  dt.getDate()
...
function forderform_check(f) {
  ...
  var buffer_ts = 30 * 60 * 1000;  // 30분 버퍼
  var rsvt_ts = f.hp_rsvt_ts.value * 1000;
  var now_ts = +new Date();
  //console.log(rsvt_ts, new Date(rsvt_ts));
  //console.log(now_ts + buffer_ts, new Date(now_ts + buffer_ts));

  if (rsvt_ts > 0 && ((now_ts + buffer_ts) > rsvt_ts)) {
    alert("잘못된 예약 시간입니다. 예약 시간을 확인해 주세요.");
    return false;
  }
  ...
  if(jQuery(f).triggerHandler("form_sumbit_order_"+form_order_method) !== false ) {
    ...
    <?php if($default['de_pg_service'] == 'kcp') { ?>
      ...
      f.pay_mod.value = "N";
      if(typeof f.escrow_use !== "undefined") {
        if (f.escrow_use.value == "Y") {
          f.pay_mod.value = "O";
        }
      }

      if (typeof f.site_name_orig !== "undefined" && f.site_name_orig.value) {
        f.site_name.value = f.site_name_orig.value;
      }

      if (typeof f.goods_orig !== "undefined" && f.goods_orig.value) {
        f.good_name.value = f.goods_orig.value;
      }

      f.eng_flag.value = "N";
      f.used_card_YN.value = "N";
      ...
      switch(settle_method) {
        ...
        var valid_ts = rsvt_ts - buffer_ts;
        var left_hour = Math.floor((valid_ts - now_ts) / 1000) / 60 / 60;
        if (rsvt_ts == 0 || left_hour > 36) {
          f.vcnt_expire_term.value = "1";
          f.vcnt_expire_term_time.value = "235959";
        } else {
          var arr_limit_dt = format_datetime(new Date(valid_ts), true);
          f.vcnt_expire_term.value = Math.ceil(((new Date(arr_limit_dt[0])).getTime()-(new Date()).getTime())/(1000*3600*24));
          f.vcnt_expire_term_time.value = arr_limit_dt[1].replaceAll(":", "");
        }
        //console.log(f.vcnt_expire_term.value, f.vcnt_expire_term_time.value);
        break;
        case "휴대폰":
        ...
        f.eng_flag.value = "Y";
        if (typeof f.site_name_eng !== "undefined" && f.site_name_eng.value) {
          f.site_name.value = f.site_name_eng.value;
        }
        if (typeof f.goods_eng !== "undefined" && f.goods_eng.value) {
          f.good_name.value = f.goods_eng.value;
        }
        break;
        case "간편결제":
        f.pay_method.value   = "100000000000";
