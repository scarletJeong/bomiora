...
if (in_array($od_settle_case, ['lpay', 'inicis_kakaopay'])) {
...
// jacknam
if (!in_array($od_settle_case, ['', 'NONE', '무통장', 'KAKAOPAY', 'TEST']) && ($default['de_pg_service'] == 'lg' && !$_POST['LGD_PAYKEY'])) {
...
if ($od_settle_case == "무통장") {
  ...
  $od_tno             = $tno;
  $od_receipt_price   = 0;
  //$od_bank_account    = $bankname.' '.$account; //입금계좌
  $od_bank_account    = $bankname . '/' . $account . '/' . $va_date; //입금은행/가상계좌번호/입금시한
  $od_deposit_name    = $depositor; //예금주
  $pg_price           = $amount;
  $od_misu            = $tot_payment - $od_receipt_price;
  $payment_request = true;
...
if ($hp_rsvt_date && $hp_rsvt_stime && $hp_rsvt_etime && $hp_doc_name) {
  $latest = ", hp_rsvt_date = '{$hp_rsvt_date}', hp_rsvt_stime = '{$hp_rsvt_stime}', hp_rsvt_etime = '{$hp_rsvt_etime}', hp_doc_name = '{$hp_doc_name}'";
} else {
  $latest = '';
}

$sql = " update {$g5['g5_shop_health_profile_cart_table']} set od_id = '{$od_id}', hp_status = '{$cart_status}' {$latest} where od_id = '{$tmp_cart_id}' ";
$result = sql_query($sql, false);
if(!$result) {
  $sql = " update {$g5['g5_shop_health_profile_cart_table']} set od_id = '{$od_id}', hp_status = '{$cart_status}' {$latest} where od_id = '{$od_id}' ";
  sql_query($sql);
}
