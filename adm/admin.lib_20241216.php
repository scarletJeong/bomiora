...
// jacknam
$admin_ips = ['122.34.110.5', '59.10.110.245', '118.235.3.98', '118.235.3.30', '124.53.31.167'];
$tmp_ip = get_real_client_ip();
$admin_key = md5($member['mb_datetime'] . get_real_client_ip() . $_SERVER['HTTP_USER_AGENT']);
// $admin_key = md5($member['mb_datetime'] . $tmp_ip . $_SERVER['HTTP_USER_AGENT']);

if (get_session('ss_mb_key') != $admin_key) {
  if (in_array($tmp_ip, $admin_ips)) {
    set_session('ss_mb_key', $admin_key);
  } else {
    session_destroy();
