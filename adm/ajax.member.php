...
function vendor_inf_list($task) {
  ...
  if ($task == 'add_inf') {
    ...
    $sql = "SELECT * FROM {$g5['member_table']} a left join {$g5['g5_shop_vendor_table']} b on a.mb_id = b.i_id
            WHERE a.mb_level = '{$_const['level']['인플루언서']}' AND a.mb_inf_code <> '' AND a.mb_id <> '{$mb_id}' and (isnull(b.v_id) or b.v_id <> '{$mb_id}') ORDER BY a.mb_datetime DESC, a.mb_id DESC;";
  } else if ($task == 'list_inf') {
    $mb_id = $_POST['mb_id'];
    if (!$mb_id) {
      return array('error' => 1, 'msg' => '필수 정보가 누락되었습니다.' );
    }
    $sql = "SELECT * FROM {$g5['g5_shop_vendor_table']} a LEFT JOIN {$g5['member_table']} b ON a.i_id = b.mb_id WHERE a.v_id = '{$mb_id}' ORDER BY a.datetime DESC, mb_no DESC;";
  } else {
    //add_vendor
    $sql = "select * from {$g5['member_table']} where mb_level = '{$_const['level']['인플루언서']}' and mb_inf_code <> '' and mb_5 <> 'y' order by mb_datetime DESC, mb_id desc;";
  }
