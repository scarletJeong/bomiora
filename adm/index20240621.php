...
// define('G5_online', 'https://bomiora.kr/bbs/board.php?bo_table=online'); 
...
$sqltest = "SELECT  qcnt,b.wr_id, b.wr_subject,b.mb_id,b.mb_name,b.mb_nick,mb_hp,mb_email,b_mb_email_certify,b_mb_certify
                    FROM (  SELECT COUNT(wr_id) AS qcnt,wr_id, wr_subject,mb_id FROM  bomiora_write_online  WHERE wr_7 IS NULL AND mb_id IS NOT null ORDER BY wr_id DESC  ) a
                            LEFT JOIN  
                            (SELECT mb_id,mb_name,mb_nick,mb_hp,mb_email,
                                    CASE WHEN mb_email_certify IS NOT null THEN 1 
                                        ELSE 0
                                        END AS b_mb_email_certify, 
                                        CASE WHEN mb_certify IS NOT null THEN 1 
                                        ELSE 0
                                        END AS b_mb_certify  FROM bomiora_member   WHERE mb_intercept_date IS not NULL AND mb_leave_date IS not NULL ) b
                            ON a.mb_id = b.mb_id";

 
$result1 = sql_query($sqltest); // 쿼리 실행 
...
<section>
    ...
    <table>
            <caption>온라인 상담 대기</caption>
            <thead>
                <tr>
                    <th scope="col">상담문의</th>
                    <th scope="col">이름</th>
                    <th scope="col">닉네임</th>
                    <th scope="col">본인인증</th>
                    <th scope="col">이메일인증</th>  
                </tr>
            </thead>
            <tbody>
                    <?php  
                        for ($i = 0; $row = sql_fetch_array($result1); $i++) {   
                             $link_url = '<a href="../bbs/board.php?bo_table=online&wr_id=' . $row['wr_id'] . '">' .  get_text($row['wr_subject']) . '</a>';
 
                    ?>
                    <tr> 

                                <td class="td_category"><?php  echo $link_url ?></td> 
                                <td class="td_mbname"><?php echo get_text($row['mb_name']); ?></td>
                                <td class="td_mbname sv_use">
                                    <div><?php echo $mb_nick ?></div>
                                </td> 
                                <td class="td_boolean"><?php echo $row['b_mb_email_certify'] ? '예' : '아니오'; ?></td>
                                <td class="td_boolean"><?php echo $row['b_mb_certify'] ? '예' : '아니오'; ?></td>   
                    </tr>
                <?php
                }
                if ($i == 0) {
...
<section>
    ...
    <div class="btn_list03 btn_list">
        <a href="./general_member_list.php">회원 전체보기</a>
    </div>
